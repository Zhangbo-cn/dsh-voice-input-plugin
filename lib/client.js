	...
	factory: (require) => {
		...
		*/
		function stripMarkdownForSpeech(text) {
			return text.replace(/```[\s\S]*?```/g, " ").replace(/`([^`]+)`/g, "$1").replace(/!\[([^\]]*)\]\([^)]*\)/g, "$1").replace(/\[([^\]]+)\]\([^)]*\)/g, "$1").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\*([^*]+)\*/g, "$1").replace(/__([^_]+)__/g, "$1").replace(/_([^_]+)_/g, "$1").replace(/~~([^~]+)~~/g, "$1").replace(/^#{1,6}\s+/gm, "").replace(/^\s*[-*+]\s+/gm, "").replace(/^\s*>\s?/gm, "").replace(/&(amp|lt|gt|quot|#39);/g, (m) => ({
				"&amp;": "&",
				"&lt;": "<",
				"&gt;": ">",
				"&quot;": "\"",
				"&#39;": "'"
			})[m] ?? m).replace(/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE0F}\u{200D}\u{20E3}]/gu, "").replace(/[\u{1F1E6}-\u{1F1FF}]{2}/gu, "").replace(/\s+/g, " ").trim();
		}
		/**
			...
			*/
			...
			return {
				...
				speak(text) {
					...
					stopAll();
					speaking = true;
					fetchImpl(`/api/tts?text=${encodeURIComponent(clean)}`).then((response) => {
		...
		* handed to TTS. An incomplete run (no sentence-end delimiter) that is followed
		* by a completed sentence is flushed as its own segment first — otherwise the
		* spoken-prefix advance would mark it read without ever speaking it (a
		* colon-terminated heading before a list gets skipped, for example). Only the
		* final trailing incomplete run stays un-spoken and is re-evaluated later.
		*/
		function splitStreamSegments(text) {
			...
			while ((match = pattern.exec(text)) !== null) {
				...
				if (SENTENCE_END.test(raw) || trimmed.length >= STREAM_FLUSH_CHARS) {
					if (buffered.trim().length > 0) result.push({
						segment: buffered.trim(),
						end: match.index
					});
					buffered = "";
					result.push({
		...
		*/
		function MicButton({ useInput, useSession, inputActions, t, language, interimResults }) {
			...
			/** The user asked to stop reading; gates the streaming and finalize effects so a still-generating reply cannot re-queue segments after the stop. */
			const stopRequestedRef = (0, react.useRef)(false);
			...
			const pumpQueue = () => {
				...
				const segmentTimer = window.setTimeout(() => {
					if (ttsSpeakingRef.current) {
						console.warn(`[dsh-voice] segment stalled; skipping: ${segment.slice(0, 20)}…`);
						ttsSpeakingRef.current = false;
						pumpQueue();
					}
				}, Math.max(3e4, segment.length * 400));
				sp.onend = () => {
			...
			(0, react.useEffect)(() => {
				if (!chatArmedRef.current || stopRequestedRef.current) return;
			...
			*/
			...
			const onPointerDown = () => {
				...
				if (readingReply) {
					...
					const wasMonitoring = monitoringRef.current;
					stopReading();
					if (!wasMonitoring) beginMonitoring();
					return;
				}
		...
		/**
		* The mic control glyph: a linear mic (blue while listening, secondary when
		* idle) or — while the reply is being read aloud — a solid red stop square,
		* the unmistakable interrupt affordance for "tap to stop".
		*/
		function MicIcon({ listening, readingReply }) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				className: "dsh-voice-icon",
				"aria-hidden": "true",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("style", { children: `@keyframes dsh-mic-pulse{0%,100%{opacity:1}50%{opacity:.45}}.dsh-voice-input{border:none;background:transparent;padding:4px;cursor:pointer;display:inline-flex;align-items:center;line-height:0;color:inherit;border-radius:6px}.dsh-voice-input:hover{background:var(--dsw-alias-interactive-bg-hover);opacity:1}.dsh-voice-icon{display:inline-flex;width:17px;height:17px;color:${listening ? DEEPSEEK_BLUE : "var(--dsw-alias-label-secondary)"}}.dsh-voice-icon svg{width:100%;height:100%}.dsh-voice-input[aria-pressed="true"] .dsh-voice-icon{animation:dsh-mic-pulse 1s ease-in-out infinite}.dsh-voice-input[data-reading] .dsh-voice-icon{color:var(--dsw-alias-state-error-primary)}` }), readingReply ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
					viewBox: "0 0 24 24",
					fill: "currentColor",
					stroke: "none",
